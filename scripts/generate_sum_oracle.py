"""Reproduce with uv run --with numpy==2.4.3 scripts/generate_sum_oracle.py.
...
def build_fixture():
    ...
    def add(shape, bits):
        ...
        with np.errstate(invalid="ignore", over="ignore"):
            expected = np.sum(source, dtype=np.float64)
    ...
    assert sequential(np.array([1e16, 1.0, -1e16, 1.0], dtype=np.float64)) == 1.0
    assert len(cases) == 100
    return {
        "numpy_version": np.__version__,
        "operation": "numpy.sum(a, dtype=numpy.float64)",
        "encoding": "unsigned binary64 bits in row-major order; NaN result bits are null",
        "contract": {
            "rust_signature": "pub fn sum(a: &Array) -> f64",
            "order": "start at positive zero; add each flat value left-to-right in f64",
            "ranks": "all ranks; scalar participates in initial addition; empty returns positive zero",
            "special_values": "IEEE infinities and overflow; NaN class only; input bits unchanged",
            "unsupported": ["axis", "keepdims", "dtype", "out", "where", "initial"],
...
def verify_rust(fixture):
    exercise = ROOT / "exercises/03_reductions/008_sum.rs"
    solution = ROOT / "solutions/03_reductions/008_sum.rs"
    assert exercise.read_text().split("#[cfg(test)]", 1)[1] == solution.read_text().split("#[cfg(test)]", 1)[1]
    with tempfile.TemporaryDirectory(prefix="sum-check-") as tmp:
        tmp = Path(tmp)
        ...
        for name, path in [("exercise", exercise), ("solution", solution)]:
            ...
            if name == "exercise":
                assert run.returncode != 0 and "not yet implemented: sum" in run.stdout, run.stdout + run.stderr
        ...
        for i, case in enumerate(fixture["cases"]):
            ...
            blocks.append("{" + f"let a = rumpy::Array::from_vec(vec![{bits}], vec![{shape}]).unwrap();"
                          ...
                          "let actual = reference::sum(&a);" + check +
...
def main():
    ...
    path = ROOT / "validation/sum-numpy.json"
    if args.check:
