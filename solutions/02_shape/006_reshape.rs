// Reference 006: compare only after your attempt.
// Owned-copy subset of numpy.reshape, with the full exercise contract tests.
...
pub fn reshape(a: &Array, shape: &[usize]) -> Result<Array, ShapeError> {
    // A reshape changes the grouping, not the flat row-major sequence.
    // Slice copies preserve every f64 bit and cannot alias the input storage.
    // The scaffold already handles scalars, empty axes, mismatches, and overflow.
