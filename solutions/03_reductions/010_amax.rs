...
pub fn amax(a: &Array) -> f64 {
    // TODO (historical, completed): Reduce the stored slice, not the shape axes.
    let (&first, rest) = a
        .as_slice()
        .split_first()
        .expect("amax requires at least one element");
    // No numeric identity works for an empty maximum. Start with a real input.
    let mut maximum = first;
    if maximum.is_nan() {
        return f64::NAN;
    }
    for &value in rest {
        // Test NaN explicitly: comparisons alone would silently ignore it.
        if value.is_nan() {
            return f64::NAN;
        }
        // Strict comparison preserves the first equal value, including its zero sign.
        // Assignment copies the selected bits. No arithmetic can round the result.
        if value > maximum {
            maximum = value;
        }
    }
