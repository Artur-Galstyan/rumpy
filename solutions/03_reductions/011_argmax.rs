...
pub fn argmax(a: &Array) -> usize {
    // TODO (historical, completed): Select a stored index, not a shape coordinate.
    let values = a.as_slice();
    // Check stored data before any traversal. Huge empty axes need no products.
    let &first = values
        .first()
        .expect("argmax requires at least one element");
    let mut best = 0;
    let mut maximum = first;
    for (index, &value) in values.iter().enumerate() {
        // Flat traversal makes the first NaN decisive, even after infinity.
        if value.is_nan() {
            return index;
        }
        // Strict comparison retains the first tie, including both zero signs.
        if value > maximum {
            maximum = value;
            best = index;
        }
