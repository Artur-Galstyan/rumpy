// Reference for lessons/012-add.md. Compare after your attempt.
use rumpy::Array;
...
pub fn add(a: &Array, b: &Array) -> Array {
    // Validate the complete shapes, not just their element counts.
    if a.shape() != b.shape() {
        panic!("add requires identical shapes");
    }
    // Equal valid shapes imply equal slice lengths. Empty slices need no axis scan.
    let data = a
        .as_slice()
        .iter()
        .zip(b.as_slice())
        .map(|(x, y)| x + y)
        .collect();
    // Reuse the checked input shape, but own a separate output buffer and shape.
