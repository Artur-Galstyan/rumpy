"""Generate real NumPy same-shape multiply fixtures; run with Python and NumPy installed."""
...
def main():
    ...
    def multiply_case(shape, left_bits, right_bits):
    ...
    for shape in [(), (1,), (7,), (2, 3), (3, 2), (1, 2, 1, 3), (2, 2, 2), (0,), (2, 0, 3)]:
        ...
        for _ in range(12):
            ...
            multiply_case(shape, bits(left), bits(right))
    ...
    for left in special:
        multiply_case((len(special),), [left] * len(special), special.copy())
    ...
    for _ in range(20):
        ...
        multiply_case((4, 4), left.tolist(), right.tolist())

    document = {
        "operation": "numpy.multiply", "numpy_version": np.__version__,
        "scope": "identical shapes only; null expected entries mean NaN class; other entries are binary64 bits",
        "generator": "python3 validation/generate_multiply_numpy.py", "seed": 13013,
        "cases": cases,
    ...
    destination = Path(__file__).with_name("multiply-numpy.json")
    destination.write_text(json.dumps(document, indent=2, allow_nan=False) + "\n")
    print(f"NumPy {np.__version__}: {len(cases)} multiply cases, {sum(len(c['expected']) for c in cases)} result values -> {destination}")
