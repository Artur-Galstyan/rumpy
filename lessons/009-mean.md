...
The previous `sum` exercise remains unfinished and has no public library API. This exercise does not depend on it.
...
The input shape, strides, and every data bit must stay unchanged.
...
Each helper call checks the unchanged input shape, strides, and data bits.
...
A future `mean` can use your public `sum` if it preserves this exact order and positive-zero start.
